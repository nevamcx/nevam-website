<template>
  <section class="faq-ask-wrapper section-padding section-bg" id="faqs">
    <div class="container">
      <div class="col-12">
        <div class="block-contents fw500">
          ...
          <div class="section-content">
            <p>Below are some of our most asked questions, and also some interesting facts about us. Also if you're not able to find an answer to a question you have, please feel to send us a message on our <a class="website-link" aria-label="Contact us page" @click.prevent="navigateTo('/contact')">contact page.</a></p>
          </div>
        </div>
      </div>
      <div class="row faq-ask">
        <template v-for="(faqParent, faqParentIndex) in faqs" :key="faqParentIndex.heading">
          <h4 class="heading">{{ faqParent.heading }}</h4>
          <div class="col-lg-6 col-12">
            <div class="faq-accordion">
              <div class="accordion" :id="`parent-accordion-${faqParentIndex}-left`">
                <div v-for="(faqChild, faqChildIndex) in faqParent.faqsLeft" class="accordion-item">
                  <h4 class="accordion-header">
                    <button 
                      class="accordion-button collapsed" 
                      aria-label="Open or close an FAQ item" 
                      type="button" 
                      data-bs-toggle="collapse"
                      :data-bs-target="`#id-${faqParentIndex}-${faqChildIndex}-left`" 
                      aria-expanded="false" 
                      :aria-controls="`#id-${faqParentIndex}-${faqChildIndex}-left`"
                    >
                      {{ faqChild.title }}
                    </button>
                  </h4>
                  <div 
                    :id="`id-${faqParentIndex}-${faqChildIndex}-left`"
          ...
          <div class="col-lg-6 col-12">
            <div class="faq-accordion">
              <div class="accordion" :id="`parent-accordion-${faqParentIndex}-right`">
                <div v-for="(faqChild, faqChildIndex) in faqParent.faqsRight" class="accordion-item">
                  <h4 class="accordion-header">
                    <button 
                      class="accordion-button collapsed" 
                      aria-label="Open or close an FAQ item" 
                      type="button" 
                      data-bs-toggle="collapse"
                      :data-bs-target="`#id-${faqParentIndex}-${faqChildIndex}-right`" 
                      aria-expanded="false" 
                      :aria-controls="`#id-${faqParentIndex}-${faqChildIndex}-right`"
                    >
                      {{ faqChild.title }}
                    </button>
                  </h4>
                  <div 
                    :id="`id-${faqParentIndex}-${faqChildIndex}-right`"
                    class="accordion-collapse collapse" 
...
const faqs = [
  {
    ...
    faqsLeft: [
      ...
      },
      {
        ...
        content: "Well, isn't this a coincidence? We've actually penned a whole 'About Us' section just for your entertainment! Dive in for a bit of light reading on our <a class='website-link' aria-label='About us page' href='/about'>about page</a>."
      },
      {
    ...
    faqsRight: [
      {
        ...
        content: "The cost depends on a few factors: the number of integrated channels and the frequency of insights you require. We're more than happy to provide you with a quote, with no obligation attached. Simply <a class='website-link' aria-label='Contact us page' href='/contact'>contact us</a> for more info."
      },
      {
